        ...
        if self.task in (Task.SQUARE, Task.TRIANGLE):
            if self.task_state == TaskState.MOVE_FORWARD:
                if current_time - self.start_time >= self.straight_duration:
                    self.current_side += 1
                    self.start_time = current_time
                    if self.task == Task.SQUARE:
                        self.change_state(TaskState.TURN_LEFT)
                    else:
                        self.change_state(TaskState.TURN_RIGHT)
                else:
                    self.go_straight(forward=True)
            elif self.task_state in (TaskState.TURN_LEFT, TaskState.TURN_RIGHT):
                turn_dur = self.turn_duration if self.task == Task.SQUARE else (pi * 2 / 3 / self.max_yaw) * 1e9
                if current_time - self.start_time >= turn_dur:
                    total_sides = 4 if self.task == Task.SQUARE else 3
                    if self.current_side < total_sides:
                        self.start_time = current_time
                        self.change_state(TaskState.MOVE_FORWARD)
                    else:
            ...
            if self.task_state == TaskState.MOVE_FORWARD:
                if current_time - self.start_time >= self.straight_duration:
                    ...
                    self.go_in_direction(
                        forward=(self.task == Task.FORWARD),
                        backward=(self.task == Task.BACKWARD),
                        left=(self.task == Task.LEFT),
                        right=(self.task == Task.RIGHT),
                        up=(self.task == Task.UP),
                        down=(self.task == Task.DOWN),
                    )
